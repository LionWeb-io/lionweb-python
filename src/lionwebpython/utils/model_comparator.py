...
class ComparisonResult:
    ...
    def mark_different_concept(self, context:str, node_id:str, concept_id_a:str, concept_id_b:str):
        self.differences.append(
            f"{context} (id={node_id}): different concepts, a={concept_id_a}, b={concept_id_b}"
        )

    def mark_different_property_value(self, context:str, node_id:str, property_name:str, value_a: object, value_b: object):
        self.differences.append(
            f"{context} (id={node_id}): different property value for {property_name}, a={value_a}, b={value_b}"
        )

    def mark_different_number_of_children(self, context: str, node_id:str, containment_name:str, children_a: int, children_b: int):
        self.differences.append(
            f"{context} (id={node_id}): different number of children for {containment_name}, a={children_a}, b={children_b}"
        )

    def mark_different_number_of_references(self, context:str, node_id:str, reference_name:str, children_a: int, children_b: int):
        self.differences.append(
...
class ModelComparator:
    ...
    def _compare_properties(self, concept: Classifier, node_a: ClassifierInstance, node_b: ClassifierInstance, comparison_result: ComparisonResult, context: str):
        for property in concept.all_properties():
            ...
            if value_a != value_b:
                comparison_result.mark_different_property_value(context, cast(str, node_a.get_id()), property.qualified_name(), value_a, value_b)

    def _compare_references(self, concept: Classifier, node_a: ClassifierInstance, node_b: ClassifierInstance, comparison_result: ComparisonResult, context: str):
    ...
    def _compare_containments(self, concept: Classifier, node_a: ClassifierInstance, node_b: ClassifierInstance, comparison_result: ComparisonResult, context: str):
        for containment in concept.all_containments():
            ...
            if len(value_a) != len(value_b):
                comparison_result.mark_different_number_of_children(context, cast(str, node_a.get_id()), containment.qualified_name(), len(value_a), len(value_b))
            else:
                for i, (child_a, child_b) in enumerate(zip(value_a, value_b)):
                    self._compare_nodes(child_a, child_b, comparison_result, f"{context}/{containment.get_name()}[{i}]")

    def _compare_annotations(self, concept: Classifier, node_a: ClassifierInstance, node_b: ClassifierInstance, comparison_result: ComparisonResult, context: str):
        if len(node_a.get_annotations()) != len(node_b.get_annotations()):
            comparison_result.mark_different_number_of_annotations(context, len(node_a.get_annotations()), len(node_b.get_annotations()))
    ...
    def _compare_nodes(self, node_a: Node, node_b: Node, comparison_result: ComparisonResult, context: str):
        if node_a.get_id() != node_b.get_id():
            comparison_result.mark_different_ids(context, cast(str, node_a.get_id()), cast(str, node_b.get_id()))
        else:
            if node_a.get_classifier().get_id() == node_b.get_classifier().get_id():
                ...
                comparison_result.mark_different_concept(
                    context, cast(str, node_a.get_id()), cast(str, node_a.get_classifier().get_id()), cast(str, node_b.get_classifier().get_id())
                )

    def _compare_annotation_instances(self, node_a: AnnotationInstance, node_b: AnnotationInstance, comparison_result: ComparisonResult, context: str):
        if node_a.get_id() != node_b.get_id():
            comparison_result.mark_different_ids(context, cast(str, node_a.get_id()), cast(str, node_b.get_id()))
        else:
            if node_a.get_annotation_definition().get_id() == node_b.get_annotation_definition().get_id():
                concept = node_a.get_annotation_definition()
                if cast(Node, node_a.get_parent()).get_id() != cast(Node, node_b.get_parent()).get_id():
                    comparison_result.mark_different_annotated(context, cast(str, node_a.get_id()), cast(str, node_b.get_id()))
